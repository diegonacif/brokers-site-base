...
export default function Header({ menuItems, logo, nomeCorretor }: HeaderProps) {
  ...
  return (
    <>
      <div className="w-full h-10 bg-brand3/100" />
      <header className="absolute top-10 left-0 bg-transparent text-white px-8 py-4 w-full h-28 z-20">
        <div className="container mx-auto flex justify-between items-center h-full">
          {/* Logo ou Nome do Corretor */}
          {logo ? (
            <Image src={logo} alt="Logo" width={225} height={50} className="object-contain" />
          ) : (
          ...
          <nav className="hidden md:flex space-x-6">
            {menuItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-lg font-[500] text-brand1 hover:text-white transition-colors">
                {item.label}
              </Link>
            ))}
          </nav>
